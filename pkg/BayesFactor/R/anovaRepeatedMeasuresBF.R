...
anovaRepeatedMeasuresBF <-
           ...
           noSample=FALSE, callback=function(...) as.integer(0))
  {
    ...
    whichRandom <- unique(c(id, whichRandom))
...
getWithinSubjectFactors <- function(data, dataTypes) { 
  ...
  for (fact in names(dataTypes[dataTypes == "fixed"])) {
    ...
    nDistinctBySubject <- aggregate(data[[fact]], by = data[id], FUN = function(x) length(unique(x)))
    if (all(nDistinctBySubject$x == nDistinct))
      withinSubjectFactors <- c(withinSubjectFactors, fact)
  }
...
addRandomSlopes <- function(models, data, dataTypes) {
  ...
  allPossibleSlopes <- labels(stats::terms(stats::as.formula(paste("~", paste(withinSubjectFactors, collapse = "*")))))
  
  # drop the most complex interaction which is unidentified
